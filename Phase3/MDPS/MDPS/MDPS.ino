...
void setup() {
  ...
  // Setup PWM for H-Bridge 1 (new API)
  ledcAttach(PIN_H1_Q3, PWM_FREQ, PWM_RESOLUTION);
  ledcWrite(PIN_H1_Q3, 255);  // 100% duty = HIGH = OFF

  ledcAttach(PIN_H1_Q4, PWM_FREQ, PWM_RESOLUTION);
  ledcWrite(PIN_H1_Q4, 255);  // 100% duty = HIGH = OFF

  // Setup PWM for H-Bridge 2 (new API)
  ledcAttach(PIN_H2_Q3, PWM_FREQ, PWM_RESOLUTION);
  ledcWrite(PIN_H2_Q3, 255);  // 100% duty = HIGH = OFF

  ledcAttach(PIN_H2_Q4, PWM_FREQ, PWM_RESOLUTION);
  ledcWrite(PIN_H2_Q4, 255);  // 100% duty = HIGH = OFF
...
void Stop() {
  ...
  delay(5);
  ledcWrite(PIN_H1_Q4, 0);
  ledcWrite(PIN_H1_Q3, 0);
  ledcWrite(PIN_H2_Q4, 0);
  ledcWrite(PIN_H2_Q3, 0);
  delay(25);
  ledcWrite(PIN_H1_Q4, 255);  // 100% = HIGH = OFF
  ledcWrite(PIN_H1_Q3, 255);  // 100% = HIGH = OFF
  ledcWrite(PIN_H2_Q4, 255);  // 100% = HIGH = OFF
  ledcWrite(PIN_H2_Q3, 255);  // 100% = HIGH = OFF
...
void forward(float desiredSpeedR, float desiredSpeedL) {
  ...
  ledcWrite(PIN_H1_Q3, 130);  // Left motor - SLOWED DOWN (was 115)
  ledcWrite(PIN_H1_Q4, 255);

  digitalWrite(PIN_H2_Q1, LOW);
  digitalWrite(PIN_H2_Q2, HIGH);
  ledcWrite(PIN_H2_Q3, 130);  // Right motor - MATCHED to left (was 115)
  ledcWrite(PIN_H2_Q4, 255);
...
void backward(float desiredSpeedR, float desiredSpeedL) {
  ...
  digitalWrite(PIN_H1_Q2, LOW);
  ledcWrite(PIN_H1_Q3, 255);
  ledcWrite(PIN_H1_Q4, 100);  // Left motor - SLOWED reverse speed (was 80)

  digitalWrite(PIN_H2_Q1, HIGH);
  digitalWrite(PIN_H2_Q2, LOW);
  ledcWrite(PIN_H2_Q3, 255);
  ledcWrite(PIN_H2_Q4, 100);  // Right motor - MATCHED to left (was 80)
...
void left() {
  ...
  // Wheel 1 (Left) - Backward
  ledcWrite(PIN_H1_Q3, 255);  // Q3 OFF
  ledcWrite(PIN_H1_Q4, 100);  // Q4 PWM - SLOWED DOWN (was 95)

  // Wheel 2 (Right) - Forward
  ledcWrite(PIN_H2_Q3, 100);  // Q3 PWM - MATCHED to left (was 95)
  ledcWrite(PIN_H2_Q4, 255);  // Q4 OFF
...
void right() {
  ...
  // Wheel 1 (Left) - Forward
  ledcWrite(PIN_H1_Q3, 110);  // Q3 PWM - SLOWED DOWN (was 95)
  ledcWrite(PIN_H1_Q4, 255);  // Q4 OFF

  // Wheel 2 (Right) - Backward
  ledcWrite(PIN_H2_Q3, 255);  // Q3 OFF
  ledcWrite(PIN_H2_Q4, 110);  // Q4 PWM - MATCHED to left (was 95)

...
class NAVCONTester:
    ...
    def run_test_scenario(self):
        """Run the test scenario in a separate thread"""
        try:
            # Wait for SNC to be ready (expect IDLE:SNC:0 packet)
            self.log_message("⏳ Waiting for SNC ready signal...", "INFO")

            # Send initial HUB packet to start the sequence
            initial_packet = SCSPacket(0, 0, 0, 0)  # IDLE:HUB:0
            self.send_packet(initial_packet, "Initial HUB packet")

            # Wait for SNC response
            start_time = time.time()
            snc_ready = False

            while self.test_running and (time.time() - start_time) < 10:  # 10 second timeout
                try:
                    msg_type, data = self.message_queue.get(timeout=0.1)
                    if msg_type == 'received_packet':
                        packet = data
                        sys_state, subsystem, ist = parse_control_byte(packet.control)
                        if subsystem == SubsystemID.SNC and sys_state == SystemState.IDLE:
                            snc_ready = True
                            self.log_message("✅ SNC ready signal received", "SUCCESS")
                            break
                except queue.Empty:
                    continue

            if not snc_ready:
                self.log_message("❌ Timeout waiting for SNC ready signal", "ERROR")
                self.stop_test()
                return

            # Begin calibration phase
            self.log_message("🛠️ Beginning calibration phase...", "INFO")

            # Send SS calibration packets
            self.send_packet(SCSPacket(112, 0, 0, 0), "SS: Start calibration")
            time.sleep(0.5)
            self.send_packet(SCSPacket(96, 10, 10, 0), "MDPS: Start calibration")
            time.sleep(0.5)
            self.send_packet(SCSPacket(97, 90, 0, 0), "MDPS: Calibration rotation")
            time.sleep(2.0)  # Wait for calibration
            self.send_packet(SCSPacket(113, 0, 0, 0), "SS: Calibration complete")

            # Wait for MAZE transition
            self.log_message("⏳ Waiting for MAZE state transition...", "INFO")
            ...
            while self.test_running and (time.time() - start_time) < 15:
                try:
                    ...
                    if msg_type == 'received_packet':
                        packet = data
                        sys_state, subsystem, ist = parse_control_byte(packet.control)
                        if subsystem == SubsystemID.SNC and sys_state == SystemState.MAZE and ist == 3:
                            maze_started = True
                            self.log_message("🎯 MAZE state with IST=3 detected - NAVCON active!", "SUCCESS")
                            break
                except queue.Empty:
                    continue

            if not maze_started:
                self.log_message("❌ Timeout waiting for MAZE state", "ERROR")
                self.stop_test()
                return

            # Execute NAVCON test sequence
            self.execute_navcon_sequence()
    ...
    def execute_navcon_sequence(self):
        """Execute the main NAVCON test sequence"""
        self.log_message("🧪 Executing NAVCON test sequence...", "INFO")

        # Test rotation scenarios based on client log
        rotation_tests = [
            {"name": "Rotation 1/2 with GREEN on the RIGHT", "packets": [
                SCSPacket(161, 90, 0, 0),   # MDPS rotation command
                SCSPacket(162, 0, 0, 0),    # MDPS stop
                SCSPacket(163, 50, 50, 0),  # MDPS forward
                SCSPacket(164, 0, 100, 0),  # MDPS speed adjustment
                SCSPacket(177, 0, 2, 0),    # SS: Green detection
                SCSPacket(178, 0, 0, 0),    # SS: Update
            ]},
            {"name": "Rotation 2/2 with GREEN on the RIGHT", "packets": [
                SCSPacket(161, 90, 0, 0),   # MDPS rotation command
                SCSPacket(162, 0, 0, 0),    # MDPS stop
                SCSPacket(163, 0, 0, 0),    # MDPS stop
                SCSPacket(164, 0, 105, 0),  # MDPS speed adjustment
                SCSPacket(177, 0, 2, 0),    # SS: Green detection
                SCSPacket(178, 0, 0, 0),    # SS: Update
            ]}
        ]
        ...
        for i, test in enumerate(rotation_tests):
            ...
            self.log_message(f"🔄 {test['name']}", "INFO")
            self.progress_var.set((i / total_tests) * 100)
            self.progress_label.config(text=f"Running: {test['name']}")

            for packet in test['packets']:
                if not self.test_running:
                    break

                self.send_packet(packet, f"Test packet: {packet}")
                time.sleep(0.2)  # Small delay between packets

                # Monitor for SNC responses
                self.monitor_snc_responses(timeout=1.0)

            # Longer delay between test phases
            time.sleep(1.0)

        # Complete the test
        self.progress_var.set(100)
        self.progress_label.config(text="Test completed successfully!")
        self.log_message("✅ NAVCON test sequence completed successfully!", "SUCCESS")
